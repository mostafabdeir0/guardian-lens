...
The cross-model replications address a distinct external-validity
question: whether the behavioral signatures are confined to the original
Gemini setting. Both core 20\% signatures reproduced in the targeted
Qwen experiment, and all three full V4 cost-response hypotheses reproduced
in the separate 1,296-call Qwen experiment. Every scene-level
confirmatory effect was in the predicted direction. Generalized and
target-activated Cue-bound prioritization also again persisted through
the maximum tested 80\% efficiency penalty.

The replication should not be interpreted as numerical invariance.
Qwen effect magnitudes were smaller than the corresponding Gemini
references, particularly for Cue-bound contrasts. Modified distractors
also activated Cue-bound behavior strongly in Qwen, whereas the Gemini
cost-response distractor curves remained below the majority-priority
threshold. The replicated result is therefore the pre-specified confirmatory
signature pattern and cost persistence, not identical response magnitude
or complete cue specificity. Because model family and serving provider changed together,
the experiments cannot separate model effects from provider-specific
serving effects.


The cross-task experiment asks whether behavioral structure measured
under one response format predicts behavior when the action structure
changes. In Gemini, the continuous Task-A allocation predictor was frozen
before the indivisible binary Task-B responses and outperformed the
profile-only baseline by 12.5 percentage points. Because that baseline
already captures the profiles' broad tendencies, PV1 shows that
scene/image-conditioned Task-A behavior contributed additional predictive
information on this fixed panel.

The Qwen experiment asks a distinct but complementary question. Rather
than fitting a Qwen-specific Task-A predictor, it applies the same ledger
frozen from the Gemini allocation experiment to Qwen Task-B responses.
The same 12.5-point incremental effect, together with PV2 and PV3,
therefore provides evidence of transfer across both response format and
model/provider setting.

Both conclusions remain bounded. Task B changes the action and response
structure but retains the same fictional organizations, visual scenes,
and researcher-controlled profiles. The experiments therefore do not
establish arbitrary behavioral generalization.

In the original Gemini evaluation, the distractor condition exposes
the complementary limitation of passive observation. Cue-bound does not
systematically activate for the visually similar distractor, supporting
specificity to the designated target in that model setting. Yet when the
true activation condition is absent, Cue-bound and Neutral can become
